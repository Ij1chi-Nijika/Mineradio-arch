...
| 下载入口 | 推荐人群 | 链接 |
| --- | --- | --- |
| 夜鹭云 | 国内用户优先 | [下载 Mineradio 1.1.1-arch-fix 本体](http://fedal.icu:11242/f/0Jhx/Mineradio-1.1.1-fix.AppImage) |

如果你是小白用户，安装时只需要下载并运行 `Mineradio-1.1.1-fix.AppImage`。不要下载 `Source code`。

## 下载或安装被拦截怎么办

小众 Electron 桌面软件、未签名安装包有时会被浏览器、Windows Defender 或 SmartScreen 提示风险。请先确认安装包来自 夜鹭云 入口，文件名是 `Mineradio-1.1.1-fix.AppImage`。

1. 浏览器下载栏提示风险时，打开下载列表，点这条下载右侧的 `...` 三个点，选择 `保留` / `仍要保留` / `显示更多` 后继续保留。
2. Windows SmartScreen 弹出蓝色拦截窗口时，点 `更多信息`，再点 `仍要运行`。
3. 如果杀毒软件明确显示木马、高危或已经隔离，不要强行运行；删除该文件后重新从夜鹭云或 GitHub Release 下载，仍然异常请带截图反馈给作者。
...
> 安全提示：`v1.0.10` 及更早旧安装包不再建议继续安装或传播，请先隔离旧安装包。请使用本页提供的 `Mineradio-1.1.1-fix.AppImage` 进行纯净安装。
